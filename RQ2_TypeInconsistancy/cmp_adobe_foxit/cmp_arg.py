...
def compare_json(dic1, dic2):
    ...
    if len(d1.keys()) != len(d2.keys()):
        flag = 1
        content += '- diff in arg length: Adobe - %s , Foxit - %s\n' % (
            len(d1.keys()), len(d2.keys()))
    for k in sumk:
        if k not in d1:
            content += '- Adobe lack key: %s\n' % (k)
            flag = 1
        elif k not in d2:
            content += '- Foxit lack key: %s\n' % (k)
            flag = 1
        else:
            tp = compare_value(d1[k], d2[k])
            if tp == False:
                flag = 1
                content += '- key:%s\tAdobe: %s\tFoxit: %s\n' % (
...
def compare_json_and_array(dic1, dic2):
    ...
    for ind in range(sumk):
        ...
        if tp == False:
            flag = 1
            content += '- ind:%d key:%s\tAdobe: %s\tFoxit: %s\n' % (
